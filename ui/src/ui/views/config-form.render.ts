...
import { getSectionMeta, t } from "../strings.js";
...
export function renderConfigForm(props: ConfigFormProps) {
  if (!props.schema) {
    return html`
      <div class="muted">${t("configSchemaUnavailable")}</div>
    `;
  }
  ...
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`
      <div class="callout danger">${t("configUnsupportedSchema")}</div>
    `;
  }
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${
            searchQuery
              ? `${t("configNoSettingsMatchPrefix")}${searchQuery}${t("configNoSettingsMatchSuffix")}`
              : t("configNoSettingsInSection")
          }
        </div>
      </div>
    `;
      ...
      ${
        subsectionContext
          ? (() => {
              ...
              const description = getConfigFieldHelp(
                [sectionKey, subsectionKey],
                hint?.help ?? node.description ?? "",
              );
          ...
          : filteredEntries.map(([key, node]) => {
              const metaFromI18n = getSectionMeta(key);
              const meta =
                metaFromI18n.label || metaFromI18n.description
                  ? metaFromI18n
                  : {
